...
| **Deposit return** (`Leergut`, `Pfandrückgabe`) | Negative price. Either accept negative prices and let them reduce the group, or have the prompt skip them. Decide explicitly; do not let a negative amount reach transaction creation if the schema forbids it. |
