...
func updateAllDatabaseTablesStructure(c *core.CliContext) error {
	...
	err = datastore.Container.UserDataStore.SyncStructs(new(models.TransactionReceipt))
	...
	err = datastore.Container.UserDataStore.SyncStructs(new(models.BudgetPlanCategoryExpectation))

	if err != nil {
		return err
	}

	log.BootInfof(c, "[database.updateAllDatabaseTablesStructure] budget plan category expectation table maintained successfully")

	err = datastore.Container.UserDataStore.SyncStructs(new(models.BudgetPlanStandingExpectation))

	if err != nil {
		return err
	}

	log.BootInfof(c, "[database.updateAllDatabaseTablesStructure] budget plan standing expectation table maintained successfully")
