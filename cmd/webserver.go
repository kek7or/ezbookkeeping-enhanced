...
func startWebServer(c *core.CliContext) error {
	...
	apiRoute.Use(bindMiddleware(middlewares.RequestLog, config))
	{
		...
		apiV1Route.Use(bindMiddleware(middlewares.APITokenIpLimit(config), config))
		{
			...
			apiV1Route.POST("/budget_plans/wishes/add.json", bindApi(api.BudgetPlans.BudgetPlanWishCreateHandler, config))
			apiV1Route.POST("/budget_plans/wishes/modify.json", bindApi(api.BudgetPlans.BudgetPlanWishModifyHandler, config))
			apiV1Route.POST("/budget_plans/wishes/assign.json", bindApi(api.BudgetPlans.BudgetPlanWishAssignHandler, config))
			apiV1Route.POST("/budget_plans/wishes/unassign.json", bindApi(api.BudgetPlans.BudgetPlanWishUnassignHandler, config))
			...
			apiV1Route.POST("/budget_plans/expectations/set_standing.json", bindApi(api.BudgetPlans.BudgetPlanStandingExpectationSetHandler, config))
