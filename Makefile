...
# The Android capture app in mobile/. Its android/ directory is generated by
# "expo prebuild" and gitignored, but it is committed to nothing and rebuilt
# from node_modules, so these targets only ever need the SDK and a JDK.
#
# ANDROID_HOME is not a persistent environment variable on this machine, so it
# is resolved here rather than assumed. cygpath turns the Windows-style
# LOCALAPPDATA into the forward-slash form both bash and Java accept.
MOBILE_DIR  := mobile
ANDROID_SDK ?= $(shell cygpath -m "$$LOCALAPPDATA" 2>/dev/null)/Android/Sdk

# Every phone made in the last decade is arm64. The x86 slices only exist for
# emulators and nearly triple the file size, so they are opt-in.
APK_ABI ?= arm64-v8a

APK := $(MOBILE_DIR)/android/app/build/outputs/apk/release/app-release.apk

.PHONY: apk
apk: ## Build the Android release APK (all ABIs: make apk APK_ABI=arm64-v8a,armeabi-v7a,x86,x86_64)
	@test -d "$(ANDROID_SDK)/platform-tools" || { \
		echo "Android SDK not found at $(ANDROID_SDK)"; \
		echo "point at it with: make apk ANDROID_SDK=C:/path/to/Sdk"; \
		exit 1; }
	@command -v java >/dev/null || { echo "java not on PATH - the Android build needs a JDK (17+)"; exit 1; }
	@test -d $(MOBILE_DIR)/node_modules || (cd $(MOBILE_DIR) && npm install --no-audit --no-fund)
	cd $(MOBILE_DIR)/android \
		&& ANDROID_HOME="$(ANDROID_SDK)" ANDROID_SDK_ROOT="$(ANDROID_SDK)" \
		   ./gradlew.bat assembleRelease -PreactNativeArchitectures=$(APK_ABI)
	@$(MAKE) --no-print-directory apk-where

.PHONY: apk-where
apk-where: ## Print where the last built APK is
	@test -f "$(APK)" || { echo "no APK built yet - run: make apk"; exit 1; }
	@echo ""
	@echo "  APK:   $$(cygpath -w "$(CURDIR)/$(APK)" 2>/dev/null || echo "$(CURDIR)/$(APK)")"
	@echo "  size:  $$(du -h "$(APK)" | cut -f1)"
	@echo "  built: $$(date -r "$(APK)" '+%Y-%m-%d %H:%M:%S')"
	@echo ""
	@echo "  copy it to the phone and tap it, or:"
	@echo "    \"$(ANDROID_SDK)/platform-tools/adb.exe\" install -r \"$(APK)\""
	@echo ""
